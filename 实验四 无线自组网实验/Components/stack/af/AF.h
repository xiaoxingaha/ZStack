...
/********************* 发送选项  ***************************/
// AF_ACK_REQUEST 要求APS应答，这是应用层的应答，只在直接发送（单播）时使用。
#define AF_ACK_REQUEST                     0x10
// AF_DISCV_ROUTE 总要包含这个选项
#define AF_DISCV_ROUTE                     0x20
#define AF_EN_SECURITY                     0x40
// AF_SKIP_ROUTING 设置这个选项将导致设备跳过路由而直接发送消息。\
...
typedef struct
{
  ...
  cId_t         *pAppInClusterList;   // 指向输入 Cluster ID列表的指针
  byte          AppNumOutClusters;    // 输出簇的个数
  cId_t         *pAppOutClusterList;  // 指向输出 Cluster ID列表的指针 
...
typedef enum
{
  afAddrNotPresent = AddrNotPresent,    // 值为0，当前地址不存在，按照绑定表进行绑定传输
  afAddr16Bit      = Addr16Bit,         // 值为2，16位短地址，指定网络地址进行单薄传输，16位
  afAddr64Bit      = Addr64Bit,         // 值为3，64位长地址，指定IEEE地址进行单薄传输，64位
  afAddrGroup      = AddrGroup,         // 值为1，组播地址，组播传输
  afAddrBroadcast  = AddrBroadcast      // 值为15，广播地址，广播传输
} afAddrMode_t;   // 地址模式
...
typedef struct
{
  ...
  afAddrMode_t addrMode;    // afAddrMode_t 是一个枚举类型，参数为地址模式
  byte endPoint;    // 指定的端点号，端点241-254为保留端点，可选范围1-240
  uint16 panId;  // used for the INTER_PAN feature指
} afAddrType_t;   // 该结构体除了指定了网络地址外，还指定了目的地址的模式
...
typedef struct
{
  ...
  afNetworkLatencyReq_t latencyReq;   // 枚举类型，必须用 noLatencyReqs填充
} endPointDesc_t;   // 终端节点描述，每个终端都需要有一个zigbee的简单描述
