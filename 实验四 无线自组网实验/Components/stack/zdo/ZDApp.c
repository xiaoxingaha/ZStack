...
void ZDO_ManytoOneFailureIndicationCB()
{
  ...
  if( zgConcentratorRC == true )  // concentrator with route cache
  {
    NLME_RouteDiscoveryRequest( 0x00, MTO_ROUTE, zgConcentratorRadius);
  }
  else
  { // Concentrator with no route cache
    NLME_RouteDiscoveryRequest( 0x00, MTO_ROUTE | NO_ROUTE_CACHE, zgConcentratorRadius);
  } 
}
