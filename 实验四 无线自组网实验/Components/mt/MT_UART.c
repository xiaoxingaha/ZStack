...
uint16  MT_UartMaxZAppBufLen=50;     //add by 13051061511 uint16  MT_UartMaxZAppBufLen; 
...
void MT_UartInit ()
{
  ...
  MT_UartMaxZAppBufLen  = 50;   //add by 1305106
      ...
      */
      if ((MT_UartZAppRxStatus == MT_UART_ZAPP_RX_READY ) && (length != 0))
      {
        ...
        if ( msg_ptr )
        {
          ...
          osal_msg_send( App_TaskID, (uint8 *)msg_ptr );   //add by 13051061511
          osal_set_event(App_TaskID, SYS_EVENT_MSG);        //add by 13051061511
        }
      }
    }
  }
